...
namespace NHSD.BuyingCatalogue.Solutions.Persistence.Repositories
{
    public sealed class PriceRepository : IPriceRepository
    {
        private const string ListCataloguePricesSql = @"
            SELECT cp.CatalogueItemId,
              ...
              FROM dbo.CataloguePrice AS cp
                   INNER JOIN dbo.PricingUnit AS pu
                           ON pu.PricingUnitId = cp.PricingUnitId
                   INNER JOIN dbo.CatalogueItem AS ci
                           ON ci.CatalogueItemId = cp.CatalogueItemId
              LEFT OUTER JOIN dbo.CataloguePriceTier AS cptr
                           ON cptr.CataloguePriceId = cp.CataloguePriceId
        ...
        public async Task<IEnumerable<ICataloguePriceListResult>> GetPricesAsync(
        ...
        public async Task<IEnumerable<ICataloguePriceListResult>> GetPriceByPriceIdQueryAsync(
            int priceId,
            CancellationToken cancellationToken)
        {
            return await dbConnector.QueryAsync<CataloguePriceListResult>(
