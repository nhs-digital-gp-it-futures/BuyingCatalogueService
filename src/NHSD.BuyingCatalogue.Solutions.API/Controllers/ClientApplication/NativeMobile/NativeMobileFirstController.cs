...
namespace NHSD.BuyingCatalogue.Solutions.API.Controllers.ClientApplication.NativeMobile
{
    ...
    public sealed class NativeMobileFirstController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult> UpdateMobileFirstAsync([Required] string id, UpdateNativeMobileFirstViewModel model) =>
            (await mediator.Send(new UpdateSolutionNativeMobileFirstCommand(id, model?.MobileFirstDesign))).ToActionResult();
    }
}
