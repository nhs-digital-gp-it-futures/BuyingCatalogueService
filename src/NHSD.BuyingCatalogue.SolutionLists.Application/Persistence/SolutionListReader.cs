...
namespace NHSD.BuyingCatalogue.SolutionLists.Application.Persistence
{
    internal sealed class SolutionListReader
    {
        ...
        public async Task<SolutionList> ListAsync(IEnumerable<ICapabilityReference> capabilityReferences, bool foundationOnly, string supplierId, CancellationToken cancellationToken)
        {
            ...
            return new SolutionList(capabilityReferences,
                await _solutionListRepository.ListAsync(foundationOnly, supplierId, cancellationToken).ConfigureAwait(false));
        }
    }
}
