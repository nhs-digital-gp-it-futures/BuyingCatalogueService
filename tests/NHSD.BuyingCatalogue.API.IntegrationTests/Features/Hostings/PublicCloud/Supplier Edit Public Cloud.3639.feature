Feature: Public Cloud
    As a Supplier
    I want to Edit the Public Cloud Section
    So that I can ensure the information is correct

Background:
    Given Organisations exist
        | Name     |
        | GPs-R-Us |
    And Suppliers exist
        | Id    | SupplierName | OrganisationName |
        | Sup 1 | Supplier 1   | GPs-R-Us         |
    And Solutions exist
        | SolutionID | SolutionName   | OrganisationName | SupplierStatusId | SupplierId |
        | Sln1       | MedicOnline    | GPs-R-Us         | 1                | Sup 1      |
    And SolutionDetail exist
        | Solution | SummaryDescription             | FullDescription   | Hosting                                                                                                                                            |
        | Sln1     | An full online medicine system | Online medicine 1 | { "PublicCloud": { "Summary": "Some summary", "URL": "www.somelink.com", "ConnectivityRequired": "This Solution requires a HSCN/N3 connection" } } |
@3639
Scenario: 1. Public Cloud is updated  
    When a PUT request is made to update the hosting-type-public-cloud section for solution Sln1
        | Summary     | URL      | ConnectivityRequired |
        | New Summary | New Link | New Connectivity     |
    Then a successful response is returned
    And SolutionDetail exist
        | Solution | SummaryDescription             | FullDescription   | Hosting                                                                                                        |
        | Sln1     | An full online medicine system | Online medicine 1 | { "PublicCloud": { "Summary": "New Summary", "URL": "New Link", "ConnectivityRequired": "New Connectivity" } } |

        
@3639
Scenario: 2. Public Cloud is updated with trimmed whitespace
     When a PUT request is made to update the hosting-type-public-cloud section for solution Sln1
         | Summary              | URL              | ConnectivityRequired    |
         | "     New Summary  " | "   New Link   " | " New Connectivity    " |
    Then a successful response is returned
    And SolutionDetail exist
         | Solution | SummaryDescription             | FullDescription   | Hosting                                                                                                        |
         | Sln1     | An full online medicine system | Online medicine 1 | { "PublicCloud": { "Summary": "New Summary", "URL": "New Link", "ConnectivityRequired": "New Connectivity" } } |

@3639
Scenario: 3. Public Cloud variables are updated to null or empty
     When a PUT request is made to update the hosting-type-public-cloud section for solution Sln1
        | Summary | URL  | ConnectivityRequired |
        | NULL    | NULL |                      |
    Then a successful response is returned
    And SolutionDetail exist
        | Solution | SummaryDescription             | FullDescription   | Hosting                |
        | Sln1     | An full online medicine system | Online medicine 1 | { "PublicCloud": { } } |

@3639
Scenario: 4. Solution is not found
    Given a Solution Sln2 does not exist
    When a PUT request is made to update the hosting-type-public-cloud section for solution Sln2
        | Summary     | URL      | ConnectivityRequired |
        | New Summary | New Link | New Connectivity     |
    Then a response status of 404 is returned 

@3639
Scenario: 5. Service Failure
    Given the call to the database to set the field will fail
    When a PUT request is made to update the hosting-type-public-cloud section for solution Sln1
        | Summary     | URL      | ConnectivityRequired |
        | New Summary | New Link | New Connectivity     |
    Then a response status of 500 is returned

@3639
Scenario: 6. Solution id is not present in the request
    When a PUT request is made to update the hosting-type-public-cloud section with no solution id
        | Summary     | URL      | ConnectivityRequired |
        | New Summary | New Link | New Connectivity     |
    Then a response status of 400 is returned
